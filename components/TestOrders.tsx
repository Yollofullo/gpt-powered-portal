...
export default function TestOrders() {
  ...
  console.log('🔍 [TestOrders.tsx] const [orders, setOrders] = useState<any[]>([]);');
  const [loading, setLoading] = useState(true);
  console.log('🔍 [TestOrders.tsx] const [loading, setLoading] = useState(true);');

  useEffect(() => {
    async function fetchOrders() {
      const { data, error } = await supabase.from("orders").select("*");
      console.log('🔍 [TestOrders.tsx] const { data, error } = await supabase.from("orders").select("*");');
