...
export default function RoleChecker() {
  ...
  console.log('🔍 [page.tsx] const [role, setRole] = useState<string | null>(null);');
  const [status, setStatus] = useState<string>("Checking role...");
  console.log('🔍 [page.tsx] const [status, setStatus] = useState<string>("Checking role...");');

  useEffect(() => {
    async function fetchRole() {
      const { data: user, error } = await supabase.auth.getUser();
      console.log('🔍 [page.tsx] const { data: user, error } = await supabase.auth.getUser();');
      if (error) return setStatus("Error fetching user");
      setRole(user?.user?.user_metadata?.role || null);
      setStatus("");
    }
    fetchRole();
  }, []);
