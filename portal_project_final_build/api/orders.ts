...
export async function GET() {
  ...
  console.log('🔍 [orders.ts] } = await supabase.auth.getSession();');
  ...
  console.log('🔍 [orders.ts] const { data: orders, error } = await supabase.from('orders').select('*');');
  ...
  console.log('🔍 [orders.ts] const sortedOrders = sortDeliveries(orders);');
...
export async function PATCH(req: Request) {
  ...
  console.log('🔍 [orders.ts] const { id, status } = await req.json();');
...
export async function POST(req: Request) {
  ...
  console.log('🔍 [orders.ts] const { item_id, quantity } = await req.json();');
  ...
  console.log('🔍 [orders.ts] const assignedOperator = operators[0].id; // Assign the first available operator');
